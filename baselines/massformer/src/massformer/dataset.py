...
from massformer.misc_utils import (
    EPS,
    flatten_lol,
    none_or_nan,
    np_one_hot,
...
def data_to_device(data_d, device, non_blocking):
    ...
    for k, v in data_d.items():
        if (
            isinstance(v, th.Tensor)
            or isinstance(v, dgl.DGLGraph)
            or isinstance(v, torch_geometric.data.Data)
        ):
...
class BaseDataset(th_data.Dataset):
    def __init__(self, *dset_types, **kwargs):
        ...
        self.mol_df = (
            self.mol_df.set_index("mol_id", drop=False)
            .sort_index()
            .rename_axis(None)
        )
    ...
    def _select_spec(self):
        ...
        # collision energy (skip for EI mode as it doesn't have collision energy)
        if self.ion_mode != "EI":
            ce_mask = ~(
                self.spec_df["ace"].isna() & self.spec_df["nce"].isna()
            )
        ...
        for dset in self.primary_dset + self.secondary_dset:
            ...
            if self.ion_mode != "EI":
                num_ces_per_group = (
                    dset_spec_df[["group_id", self.ce_key]]
                    .groupby(by="group_id")
                    .count()[self.ce_key]
                    .mean()
                )
                ...
                print(f">>> {dset}")
                print(
                    f"> num_spec = {num_specs}, num_mol = {num_mols}, num_group = {num_groups}"
                )
        # subsample
        ...
        self.mol_df = self.mol_df[
            self.mol_df["mol_id"].isin(self.spec_df["mol_id"])
        ]
        self.mol_df = self.mol_df.reset_index(drop=True)

    def _setup_ce(self):
        # Skip collision energy setup for EI mode (no collision energy in EI)
        if self.ion_mode == "EI":
            # Set a default value for EI spectra (will be ignored in ce_func)
            self.spec_df[self.ce_key] = 0.0
            return
        ...
        if self.convert_ce:
            ...
            assert (
                not self.spec_df.loc[convert_mask, other_ce_key].isna().any()
            )
            self.spec_df.loc[convert_mask, self.ce_key] = self.spec_df[
                convert_mask
            ].apply(ce_conversion_fn, axis=1)
            assert not self.spec_df[self.ce_key].isna().any()
        else:
            self.spec_df = self.spec_df.dropna(
                axis=0, subset=[self.ce_key]
    ...
    def _setup_spec_metadata_dicts(self):
        ...
        self.inst_type_c2i = {
            string: i for i, string in enumerate(inst_type_list)
        }
        self.inst_type_i2c = {
            i: string for i, string in enumerate(inst_type_list)
        }
        self.prec_type_c2i = {
            string: i for i, string in enumerate(prec_type_list)
        }
        self.prec_type_i2c = {
            i: string for i, string in enumerate(prec_type_list)
        }
        self.frag_mode_c2i = {
            string: i for i, string in enumerate(frag_mode_list)
        }
        self.frag_mode_i2c = {
            i: string for i, string in enumerate(frag_mode_list)
        }
    ...
    def bin_func(self, mzs, ints, return_index=False):
        assert self.ints_thresh == 0.0, self.ints_thresh
    ...
    def transform_func(self, spec):
        if self.process_spec_old:
            ...
            spec = spec_utils.process_spec(
                th.as_tensor(spec), self.transform, self.spectrum_normalization
            )
            spec = spec.numpy()
        return spec

    def get_predefined_split_masks(self):
        """Get split masks from pre-defined 'split' column in spec_df.

        Returns:
            tuple: (train_mask, val_mask, test_mask, sec_masks) or None if no split column exists
        """
        if "split" not in self.spec_df.columns:
            return None

        print("> Using pre-defined splits from 'split' column")
        train_mask = (self.spec_df["split"] == "train").values
        val_mask = (self.spec_df["split"] == "val").values
        test_mask = (self.spec_df["split"] == "test").values

        # Check for secondary datasets (not applicable for NIST23 but keep for compatibility)
        sec_masks = []

        print("> pre-defined splits")
        print("splits: train, val, test")
        print(f"spec: {train_mask.sum()}, {val_mask.sum()}, {test_mask.sum()}")

        # Verify no overlap
        assert (train_mask & val_mask & test_mask).sum() == 0, (
            "Splits have overlapping data!"
        )
    ...
    def get_split_masks(
        ...
        with np_temp_seed(split_seed):
            ...
            sec_key_list = np.random.choice(
                sec_key_list, size=sec_num, replace=False
            ).tolist()
            sec_key = set(sec_key_list)
            # print(sec_key_list[:5])
            sec_mol_id = self.mol_df[
                self.mol_df[split_key].isin(sec_key_list)
                & self.mol_df["mol_id"].isin(sec_mol_id)
            ]["mol_id"].unique()
        ...
        prim_only_no_casmi_key_list = [
            k for k in prim_only_key_list if k not in prim_only_and_casmi
        ]
        ...
        with np_temp_seed(split_seed):
            ...
            val_key = set(
                np.random.choice(
                    sorted(list(train_val_key)), size=val_num, replace=False
                )
            )
        ...
        train_mol_id = self.mol_df["mol_id"][
            self.mol_df[split_key].isin(list(train_key))
        ].unique()
        val_mol_id = self.mol_df["mol_id"][
            self.mol_df[split_key].isin(list(val_key))
        ].unique()
        test_mol_id = self.mol_df["mol_id"][
            self.mol_df[split_key].isin(list(test_key))
        ].unique()
        ...
        sec_masks = [
            (self.spec_df["dset"] == dset)
            & (self.spec_df["mol_id"].isin(sec_mol_id))
            for dset in self.secondary_dset
        ]
        assert (train_mask & val_mask & test_mask).sum() == 0
        print("> primary")
    ...
    def ce_func(self, col_energy):
        if self.preproc_ce == "normalize":
            col_energy_meta = th.tensor(
                [(col_energy - self.mean_ce) / (self.std_ce + EPS)],
                dtype=th.float32,
            )
    ...
    def get_spec_feats(self, spec_entry):
        ...
        prec_mz_idx = th.tensor(min(prec_mz_bin, spec.shape[1] - 1)).unsqueeze(
            0
        )
        assert prec_mz_idx < spec.shape[1], (
            prec_mz_bin,
            prec_mz_idx,
            spec.shape,
        )
        ...
        inst_type_meta = th.as_tensor(
            np_one_hot(inst_type_idx, num_classes=self.num_inst_type),
            dtype=th.float32,
        )
        prec_type_meta = th.as_tensor(
            np_one_hot(prec_type_idx, num_classes=self.num_prec_type),
            dtype=th.float32,
        )
        frag_mode_meta = th.as_tensor(
            np_one_hot(frag_mode_idx, num_classes=self.num_frag_mode),
            dtype=th.float32,
        )
        spec_meta_list = [
            col_energy_meta,
    ...
    def get_dataloaders(self, run_d):
        ...
        # First try to use pre-defined splits from the 'split' column
        split_result = self.get_predefined_split_masks()
        if split_result is not None:
            train_mask, val_mask, test_mask, sec_masks = split_result
            print("✓ Using pre-defined splits")
        else:
            print(
                "⚠ No pre-defined splits found, computing splits dynamically"
            )
        ...
        sec_ss = [
            th_data.Subset(self, all_idx[sec_mask]) for sec_mask in sec_masks
        ]
        ...
        split_id_dict["primary"]["train"] = self.spec_df.iloc[
            all_idx[train_mask]
        ]["spec_id"].to_numpy()
        split_id_dict["primary"]["val"] = self.spec_df.iloc[all_idx[val_mask]][
            "spec_id"
        ].to_numpy()
        split_id_dict["primary"]["test"] = self.spec_df.iloc[
            all_idx[test_mask]
        ]["spec_id"].to_numpy()
        split_id_dict["secondary"] = {}
        for sec_idx, sec_dset in enumerate(self.secondary_dset):
            split_id_dict["secondary"][sec_dset] = self.spec_df.iloc[
                all_idx[sec_masks[sec_idx]]
            ]["spec_id"].to_numpy()
    ...
    def get_track_dl(
        ...
        if spec_ids is not None:
            ...
            for spec_id in spec_ids:
                spec_idx.append(
                    int(
                        self.spec_df[self.spec_df["spec_id"] == spec_id].index[
                            0
                        ]
                    )
                )
    ...
    def compute_lda(self, run_d):
        ...
        # untransform
        train_spec = spec_utils.unprocess_spec(
            train_spec, self.transform
        ).numpy()
        # lda transform
        ...
        lda_topic = np.concatenate(
            [
                lda_topic,
                np.zeros((other_spec_id.shape[0], lda_topic.shape[1])),
            ],
            axis=0,
        )
        lda_topic = [
            row[0] for row in np.split(lda_topic, lda_topic.shape[0], axis=0)
        ]
        # add them to the spec_df
        lda_df = pd.DataFrame({"spec_id": spec_id, "lda_topic": lda_topic})
        self.spec_df = self.spec_df.merge(lda_df, on=["spec_id"], how="inner")
        ...
        def _collate(data_ds):
            ...
            for k, v in batch_data_d.items():
                if isinstance(data_ds[0][k], th.Tensor):
                    ...
                    batch_data_d[k] = gf_data_utils.collator(v)
                else:
    ...
    def process_entry(self, spec_entry, mol):
        ...
        if self.is_fp_dset:
            ...
            if "maccs" in self.fp_types:
                ...
                fps.append(fp)
            fps = th.cat(
                [th.as_tensor(fp, dtype=th.float32) for fp in fps], dim=0
            ).unsqueeze(0)
            data["fp"] = fps
        if self.is_graph_dset:
            ...
            if self.spec_meta_node:
                assert self.atom_feature_mode not in ["pretrain"], (
                    self.atom_feature_mode
                )
    ...
    def get_atom_featurizer(self):
        ...
        if self.atom_feature_mode == "canonical":
            ...
            atom_featurizer_funs = chemutils.ConcatFeaturizer(
                [chemutils.atom_mass, data_utils.atom_type_one_hot]
            )
    ...
    def update_casmi_info(self, casmi_ds):
        query_mol_df = casmi_ds.spec_df[["mol_id"]].merge(
            casmi_ds.mol_df[["mol_id", "inchikey_s", "scaffold"]],
            on=["mol_id"],
            how="inner",
        )
...
class CASMIDataset(BaseDataset):
    def __init__(self, ds, casmi_type, *dset_types, **kwargs):
        ...
        self.spec_df = pd.read_pickle(
            os.path.join(self.proc_dp, casmi_dp, "spec_df.pkl")
        )
        self.mol_df = pd.read_pickle(
            os.path.join(self.proc_dp, casmi_dp, "mol_df.pkl")
        )
        self.cand_df = pd.read_pickle(
            os.path.join(self.proc_dp, casmi_dp, "cand_df.pkl")
        )
        if casmi_type == "pcasmi":
            self.s_df = pd.read_pickle(
                os.path.join(self.proc_dp, casmi_dp, "s_df.pkl")
            )
        # select the spectra
        self.spec_df = self.spec_df[
            self.spec_df["ion_mode"] == "P"
        ].reset_index(drop=True)
        if (
            casmi_type in ["casmi", "casmi22"]
            and getattr(self, f"{casmi_type}_num_entries") > -1
        ):
            assert (
                getattr(self, f"{casmi_type}_num_entries")
                <= self.spec_df.shape[0]
            )
            self.spec_df = self.spec_df.sample(
                n=getattr(self, f"{casmi_type}_num_entries"),
                replace=False,
                random_state=6699,
            )
        elif casmi_type == "pcasmi" and self.pcasmi_num_entries > -1:
            assert (
                self.pcasmi_num_entries <= self.spec_df["group_id"].nunique()
            )
            ...
            for mw_range_idx, mw_range in enumerate(
                ...
                group_ids.extend(
                    mw_sample_df.sample(n=n, replace=False, random_state=6699)[
                        "group_id"
                    ].tolist()
                )
            assert len(set(group_ids)) == self.pcasmi_num_entries
            self.spec_df = self.spec_df[
                self.spec_df["group_id"].isin(group_ids)
            ]
            # random sampling
        ...
        query_mol_id = (
            set(self.spec_df["mol_id"])
            & set(self.mol_df["mol_id"])
            & set(self.cand_df["query_mol_id"])
        )
        cand_mol_id = set(
            self.cand_df[self.cand_df["query_mol_id"].isin(query_mol_id)][
                "candidate_mol_id"
            ]
        )
        ...
        self.mol_df = self.mol_df[
            self.mol_df["mol_id"].isin(query_mol_id | cand_mol_id)
        ]
        match_mol = self.mol_df[self.mol_df["mol_id"].isin(query_mol_id)]
        self.mol_df = self.mol_df[
            ~self.mol_df["inchikey_s"].isin(match_mol["inchikey_s"])
        ]
        ...
        self.spec_df = self.spec_df[
            self.spec_df["mol_id"].isin(query_mol_id)
        ].reset_index(drop=True)
        self.mol_df = self.mol_df[
            self.mol_df["mol_id"].isin(query_mol_id | cand_mol_id)
        ].reset_index(drop=True)
        self.cand_df = self.cand_df[
            self.cand_df["query_mol_id"].isin(query_mol_id)
            & self.cand_df["candidate_mol_id"].isin(cand_mol_id)
        ].reset_index(drop=True)
        # sanity checks
        ...
        self.spec_df = self.spec_df[
            self.spec_df["spec_id"].isin(self.mol_spec_df["query_spec_id"])
        ]
        # set indices for quicker access
        self.spec_df = (
            self.spec_df.set_index("spec_id", drop=False)
            .sort_index()
            .rename_axis(None)
        )
        self.mol_df = (
    ...
    def get_dataloader(self, run_d, mode, group_id=None):
        if mode == "spec":
            ...
            if num_workers == -1:
                num_workers = run_d["num_workers"]
            pin_memory = (
                run_d["pin_memory"] if run_d["device"] != "cpu" else False
            )
...
class CASMIGroupDataset(th_data.Dataset):
    def __init__(self, casmi_ds, group_id=None):
        ...
        if group_id is not None:
            ...
            self.mol_spec_df = self.mol_spec_df[
                self.mol_spec_df["query_spec_id"].isin(self.spec_df["spec_id"])
            ]
            self.mol_df = self.mol_df[
                self.mol_df["mol_id"].isin(
...
def get_default_ds(data_d_ow=dict(), model_d_ow=dict(), run_d_ow=dict()):
    ...
    _, _, _, data_d, model_d, run_d = load_config(
        template_fp, custom_fp, device_id, checkpoint_name
    )
...
def get_dataloader(data_d_ow=dict(), model_d_ow=dict(), run_d_ow=dict()):
    data_d, model_d, run_d = get_default_ds(
        data_d_ow=data_d_ow, model_d_ow=model_d_ow, run_d_ow=run_d_ow
    )
