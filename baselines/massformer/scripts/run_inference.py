...
def init_from_smiles(dataset, smiles_df, prec_types, nces, run_d):
    ...
    # Detect if this is EI mode
    is_ei_mode = ds.ion_mode == "EI"
    ...
    def calculate_prec_mz(prec_type, exact_mw):
        if is_ei_mode:
            # For EI mode, prec_mz is just the molecular weight
            return exact_mw
    ...
    # Handle EI mode differently - no collision energies or precursor types
    if is_ei_mode:
        rows = []
        for i in range(num_mol):
            ...
            row_d["prec_type"] = "EI"
            row_d["prec_mz"] = mol_df["exact_mw"].iloc[i]
            row_d["ace"] = np.nan
            row_d["nce"] = 0.0
            row_d["inst_type"] = ds.inst_type[0] if len(ds.inst_type) > 0 else "EI-GC"
            row_d["frag_mode"] = ds.frag_mode[0] if len(ds.frag_mode) > 0 else "EI"
            row_d["spec_type"] = "EI"
            row_d["ion_mode"] = "EI"
            row_d["dset"] = "new"
            row_d["peaks"] = [(100.,0.2),(200.,0.3),(500.,0.5)] # dummy spectrum
            row_d["res"] = ds.res[0] if len(ds.res) > 0 else 1
            row_d["col_gas"] = np.nan
            row_d["ri"] = np.nan
            rows.append(row_d)
    else:
        # LC-MS/MS mode
