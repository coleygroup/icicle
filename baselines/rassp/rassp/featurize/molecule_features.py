...
from .util import get_nos, get_nos_coords

RDLogger.DisableLog("rdApp.*")


def get_formula(mol):
    """Get molecular formula as a dict of {atomic_num: count}."""
    from collections import Counter

    formula = Counter()
    for atom in mol.GetAtoms():
        formula[atom.GetAtomicNum()] += 1
    return dict(formula)


def num_unique_frag_formulae(mol):
    """
    Compute number of unique fragment formulae for a molecule.
    This is the product of (count + 1) for each element type.
    """
    formula = get_formula(mol)
    import numpy as np

    return np.prod([v + 1 for v in formula.values()])
...
def feat_tensor_mol(
    ...
    if len(frag_props_config) > 0:
        res_mats.append(
            get_single_bond_fragment_masses(mol, **frag_props_config)
        )
...
def recon_features_edge(
    ...
    if len(a_sub) == 0:
        return np.zeros(
            (mol.GetNumAtoms(), mol.GetNumAtoms(), 0), dtype=np.float32
        )
...
def get_single_bond_fragment_masses(mol, **config):
    ...
    for bi, b in enumerate(mol.GetBonds()):
        ...
        frags = Chem.GetMolFrags(
            mol_f, sanitizeFrags=False, fragsMolAtomMapping=out_frags
        )
...
class PossibleFormulaFeaturizer:
    def __init__(
        ...
        if enumerator_type == "naive":
            self.ffe = masscompute.FragmentFormulaEnumerator(
        ...
        if isinstance(formula_max_num_atomicno, int):
            # constant int
            formula_max_num_atomicno = [formula_max_num_atomicno] * len(
                formula_possible_atomicno
            )
        # create one-hot luts
        running_pos = 0
        ...
        self.oh_offsets = np.array(
            [self.oh_an_lut[a] for a in self.formula_possible_atomicno]
        )
    ...
    def __call__(self, mol, formulae=None, masses=None):
        ...
        if len(formulae) > self.max_formulae:
            ...
            raise ValueError(
                f"molecule {Chem.MolToSmiles(mol)} has {len(formulae)}, more than the limit of {self.max_formulae} (naive count={self.num_unique_f(mol)})"
            )

        src_masses_n = masses.shape[0]
        masses = np.pad(
            masses, ((0, self.max_formulae - src_masses_n), (0, 0), (0, 0))
        )
        ...
        if self.featurize_mode == "numerical":
            ...
            formulae_feat = np.pad(
                formulae_feat,
                [(0, self.max_formulae - formulae_feat.shape[0]), (0, 0)],
            )

            return formulae_feat, masses

        elif self.featurize_mode == "onehot":
            formulae_feat = np.zeros(
                (self.max_formulae, self.oh_max), dtype=np.float32
            )

            util.fast_multi_onehot(
                formulae, self.oh_offsets, formulae_feat, accum=False
            )
            return formulae_feat, masses
            ...
            util.fast_multi_onehot(
                formulae, self.oh_offsets, formulae_feat, accum=True
            )
            return formulae_feat, masses
