...
def feat_tensor_atom(
    ...
    if DEBUG_fchl:
        ...
        fchl_rep = qml.fchl.generate_representation(
            coords, atomic_nos, max_size=64
        )
    ...
    for i in range(mol.GetNumAtoms()):
        ...
        if feat_atomicno_onehot is not None:
            ...
            logger.debug(
                f"adding {len(to_append)} feats for feat_atomicno_onehot"
            )
            atom_feature += to_append
        ...
        if total_valence_onehot:
            ...
            logger.debug(
                f"adding {len(to_append)} feats for total_valence_onehot"
            )
            atom_feature += to_append
        ...
        if default_valence:
            to_append = to_onehot(
                pt.GetDefaultValence(atomic_num), range(1, 7)
            )
        ...
        if electronegativity:
            ...
            logger.debug(
                f"adding {len(to_append)} feats for electronegativity"
            )
            atom_feature += to_append
        ...
        if mmff_atom_types_onehot:
            if mmff_p is None:
                ...
                to_append = to_onehot(
                    mmff_p.GetMMFFAtomType(i), MMFF94_ATOM_TYPES
                )

            logger.debug(
                f"adding {len(to_append)} feats for mmff_atom_types_onehot"
            )
            atom_feature += to_append
        ...
        if total_h or total_h_oh:
            attached_h = np.sum(
                [an.GetAtomicNum() == 1 for an in a.GetNeighbors()]
            )
