...
from .. import msutil
from . import atom_features, molecule_features, util


def create_mol_featurizer(spect_bin_config, featurizer_config):
    return MolFeaturizer(bin_config=spect_bin_config, **featurizer_config)


def create_pred_featurizer(spect_bin_config, pred_featurizer_config):
    return PredFeaturizer(
        bin_config=spect_bin_config, **pred_featurizer_config
    )
...
class MolFeaturizer(torch.utils.data.Dataset):
    def __init__(
        ...
        if explicit_formulae_config != {}:
            self.pff = molecule_features.PossibleFormulaFeaturizer(
                **explicit_formulae_config
            )
        else:
            self.pff = None
    ...
    def __call__(self, mol):
        ...
        f_vect = atom_features.feat_tensor_atom(
            mol, conf_idx=0, **self.feat_vert_args
        )
        ...
        adj_oh[:, : adj_oh_nopad.shape[1], : adj_oh_nopad.shape[2]] = (
            adj_oh_nopad
        )
        ...
        if self.pff is not None:
            ...
            formulae_peaks_mass_idx, formulae_peaks_intensity = self.mp2b(
                formulae_peaks
            )

            v["formulae_features"] = formulae_feats.astype(np.uint8)
            v["formulae_peaks"] = formulae_peaks
            v["formulae_peaks_mass_idx"] = formulae_peaks_mass_idx.astype(
                np.int64
            )
        ...
        if len(self.element_oh) > 0:
            element_oh_mat = np.zeros(
                (self.MAX_N, len(self.element_oh)), dtype=np.float32
            )
        ...
        if self.vert_subset_samples_n > 0:
            ...
            vert_subset_generator = create_subset_generator(
                **self.subset_gen_config
            )
            ...
            atom_subsets[:number_of_subsets, :DATA_N] = vertsubsets[
                np.random.permutation(len(vertsubsets))[:number_of_subsets]
            ]

            atom_subsets_formula_idx = (
                util.get_subset_peaks_idx_from_formulae_fast(
                    formulae_features, vert_element_oh, atom_subsets
                )
            )
            ...
            v["atom_subsets_peaks_mass_idx"] = formulae_peaks_mass_idx[
                atom_subsets_formula_idx
            ].astype(np.int64)
            v["atom_subsets_peaks_intensity"] = formulae_peaks_intensity[
                atom_subsets_formula_idx
            ]
            v["atom_subsets_formula_idx"] = atom_subsets_formula_idx.astype(
                np.int64
            )
...
class PredFeaturizer:
    ...
    def __call__(self, mol, sparse_spect):
        ...
        spect_idx, spect_p, spect_out = self.spect_bin_config.histogram(
            record_spect[:, 0], record_spect[:, 1]
        )
    ...
    if name == "BandR":
        ...
        def run(mol):
            if mol.GetNumAtoms() <= 64:
                return msutil.vertsubsetgen.BreakAndRearrangeFast(**config)(
                    mol
                )
            else:
                # fallback to the slow version
                return msutil.vertsubsetgen.BreakAndRearrangeAllH(**config)(
                    mol
                )
