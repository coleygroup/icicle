...
class RenameUnpickler(pickle.Unpickler):
    """Fix old module paths in pickled files."""
    ...
    def find_class(self, module, name):
        ...
        replacements = {
            "msutil": "rassp.msutil",
            "model": "rassp.model",
            "featurize": "rassp.featurize",
            "dataset": "rassp.dataset",
        }

        # Check if module needs to be replaced
...
MODELS = {
    "FormulaNet": {
        "checkpoint": os.path.join(
            model_dir,
            "best_config.68094069.00000000.model",
        ),
        "meta": os.path.join(
            model_dir,
            "best_config.68094069.meta",
        ),
...
FILE_TYPES = ["smiles", "inchi", "sdf", "rdmol", "parquet"]
...
class Predictor:
    ...
    def is_valid_mol(
        ...
        try:
            ...
            anum_set = set(
                [m.GetAtomWithIdx(i).GetAtomicNum() for i in range(n_atoms)]
            )
            ...
            if m is not None:
                try:
                    ...
                    logger.debug(
                        f"failed to convert mol to smiles with exception {e2}"
                    )
    ...
    def load_model(
        ...
        if use_gpu and not torch.cuda.is_available():
            logger.warning(
                "CUDA requested but not available, running with CPU"
            )
            use_gpu = False
    ...
    def predict(
        ...
        assert model_name in MODELS, (
            f"{model_name} not found in {MODELS.keys()}"
        )
        predictor, meta = Predictor.load_model(
            model_name, use_gpu=self.use_gpu
        )
        ...
        assert len(out_records) == len(mols), "len(out_records) != len(mols)"
...
def run_on_file(
    ...
    assert os.path.exists(input_filename), (
        f"input file not found at {input_filename}"
    )

    # Initialize input_df for parquet output support
    input_df = None
    ...
    if file_type == "smiles":
        mols = [
            Chem.AddHs(m)
            for m in Chem.SmilesMolSupplier(input_filename, titleLine=0)
        ]
        ...
        # Use inchi to create molecules
        mols = [
            Chem.AddHs(Chem.MolFromInchi(inchi)) for inchi in input_df["inchi"]
        ]
    else:
    ...
    click.secho("Initializing Predictor", fg="blue")
    predictor = Predictor(
        use_gpu=gpu,
    ...
    click.secho("Running RASSP forward model", fg="blue")
    all_results, pred_metadata = predictor.predict(mols, model_name=model_name)

    click.secho("Writing outputs to disk", fg="blue")
    n_empty_results = len(
        list(filter(lambda x: not x["success"], all_results))
    )
    ...
    if output_file_type == "txt":
        ...
        json.dump(
            {"predictions": all_results, "runtime": t2 - t1},
        ...
        if input_df is None:
            raise ValueError(
                "Parquet output requires parquet input (use --file-type parquet)"
            )

        # Build output dataframe preserving input metadata
        out_records = []
        for i, result in enumerate(all_results):
            record = {
                "mol_id": input_df.iloc[i]["mol_id"],
                "smiles": input_df.iloc[i]["smiles"],
                "inchi": input_df.iloc[i]["inchi"],
                "inchi_key": input_df.iloc[i]["inchi_key"],
                "morgan4_crc32": input_df.iloc[i]["morgan4_crc32"],
                "n_atoms": input_df.iloc[i]["n_atoms"],
                "mol_mass": input_df.iloc[i]["mol_mass"],
                "n_peaks": input_df.iloc[i]["n_peaks"],
                "n_formulae": input_df.iloc[i]["n_formulae"],
                "spect_true": input_df.iloc[i]["spect"],
                "spect_pred": result["spect"],
                "success": result["success"],
            }
            out_records.append(record)

        out_df = pd.DataFrame(out_records)
        out_df.to_parquet(output_filename, index=False, engine="pyarrow")
...
@click.option(
    "--file-type", help="Input file type", type=click.Choice(FILE_TYPES)
)
@click.option(
    "--output-file-type",
    help="Output file type",
    default="txt",
    type=click.Choice(["txt", "json", "parquet"]),
)
@click.option(
    "--model-name",
    help="Model name",
...
@click.option(
    "--num-data-workers", help="number of data workers to use", default=4
)
def run_predictions(
    infile,
    outfile,
