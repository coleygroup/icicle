...
## Using RASSP with ICICLE Evaluation Framework

The RASSP model can be evaluated using the ICICLE evaluation framework through the `RASSPWrapper` class. This wrapper allows you to run similarity metrics and retrieval experiments on RASSP predictions.

### Prerequisites

1. **Train or obtain a pretrained RASSP model:**
   - Follow the training instructions above to train a model, OR
   - Download pretrained model weights (both `.model` and `.meta` files) from [https://people.cs.uchicago.edu/~ericj/rassp/](https://people.cs.uchicago.edu/~ericj/rassp/)
   - Place model files in `baselines/rassp/rassp/models/`

2. **Ensure RASSP is installed:**
   ```bash
   cd baselines/rassp
   pip install -e .
   ```
...
Create or edit the evaluation config at `examples/configs/eval/rassp.yaml`:
...
model:
  architecture:
    _target_: icicle.models.baselines.rassp_wrapper.RASSPWrapper
    # Update these paths to point to your trained model
    checkpoint_path: baselines/rassp/rassp/models/best_config.XXXXXX.model
    meta_path: baselines/rassp/rassp/models/best_config.XXXXXX.meta
    model_name: "FormulaNet"  # or "SubsetNet"
    min_mz: 0
    max_mz: 1000
    bin_width: 1.0
...
Once configured, you can run the evaluation pipeline:

```bash
# Run similarity metrics
python -m icicle.eval eval=rassp similarity.enable=true

# Run retrieval with formula
python -m icicle.eval eval=rassp retrieval_with_formula.enable=true

# Run retrieval with retention index
python -m icicle.eval eval=rassp retrieval_with_ri.enable=true

# Run all evaluations
python -m icicle.eval eval=rassp \
  similarity.enable=true \
  retrieval_with_formula.enable=true \
  retrieval_with_ri.enable=true
```

### Wrapper Implementation Details

The `RASSPWrapper` class (`src/icicle/models/baselines/rassp_wrapper.py`):
- Inherits from `BaseSpectrumPredictor` to integrate with ICICLE's evaluation pipeline
- Loads RASSP models using `netutil.PredModel`
- Implements `predict_from_smiles()` interface for spectrum prediction
- Validates molecules against RASSP's constraints (max atoms, max formula)
- Handles invalid SMILES by returning empty predictions (skipped by eval pipeline)
- Supports both FormulaNet and SubsetNet model variants

### Model Constraints

RASSP models have molecule size constraints based on training:
- **FormulaNet**: Up to 48 atoms, up to 4096 unique fragment formulae
- **SubsetNet**: Up to 64 atoms, up to 49152 unique fragment formulae

The wrapper automatically validates molecules against these constraints.
