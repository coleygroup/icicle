"""Featurize integers into vectors."""

import numpy as np
import torch
from torch import nn

from icicle.utils import NORM_VEC


class IntFeaturizer(nn.Module):
    """Base class for mapping integers to a vector representation (primarily to
    be used as a "richer" embedding for NNs processing integers).
    ...
    def forward(self, tensor):
        """Convert the integer `tensor` into its new representation -- it gets
        stacked along final dimension."""
        device = tensor.device
        orig_shape = tensor.shape
        out_tensor = torch.empty(
            (*orig_shape, self.embedding_dim), device=device
        )
        ...
        # Move parameter matrices to correct device if needed
        if self.int_to_feat_matrix.device != device:
            self.int_to_feat_matrix.data = self.int_to_feat_matrix.data.to(
                device
            )
        if self._extra_embeddings.device != device:
            self._extra_embeddings.data = self._extra_embeddings.data.to(
                device
            )

        norm_embeds = self.int_to_feat_matrix[tensor[~extra_embed]]
        extra_embeds = self._extra_embeddings[
            tensor[extra_embed] - self.MAX_COUNT_INT
        ]
    ...
    * we'll put the frequencies at powers of 1/2 rather than random Gaussian samples; this means it will match the
    ...
    def __init__(self):
        ...
        all_features = torch.cat(
            [
                torch.cos(combo_of_sinusoid_args),
                torch.sin(combo_of_sinusoid_args),
            ],
            dim=1,
        )
...
class RBFFeaturizer(IntFeaturizer):
    """A featurizer that puts radial basis functions evenly between 0 and
    max_count-1.

    These will have a width of (max_count-1) / (num_funcs) to decay to about
    0.6 of its original height at reaching the next func.
    """
    ...
    def __init__(self, num_funcs=32):
        ...
        pre_exponential_terms = (
            -0.5
            * (
                (torch.arange(self.MAX_COUNT_INT)[:, None] - centers[None, :])
                / width
            )
            ** 2
        )
...
class FloatFeaturizer(IntFeaturizer):
    ...
    def forward(self, tensor):
        """Convert the integer `tensor` into its new representation -- note
        that it gets stacked along final dimension."""
