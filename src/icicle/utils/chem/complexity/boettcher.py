...
class BottchScorer:
    ...
    def score(self, mol):
        ...
        if mol.NumAtoms() == 0:
            print(
                "*** WARNING: NO ATOMS FOR MOLECULE [%s] ***" % mol.GetTitle()
            )
            return
        self.mol = mol
    ...
    def _calculate_score(self):
        ...
        for idx, eq_groups in list(self._equivalents.items()):
            for e in eq_groups:
                total_complexity -= (
                    0.5 * self._indices[idx]["complexity"] / (len(eq_groups))
                )
    ...
    def _calculate_complexity(self, idx):
        ...
        if self.verbose:
            print(
                (
                    "complexty [%3d]:  %d * %d * %d log2( %d * %d) = %2.1f"
                    % (
                        idx,
                        data["di"],
                        data["ei"],
    ...
    def _calc_di(self, idx, atom):
        ...
        # to be used as main
        if idx not in self._equivalents:
            if idx in self.automorphs:
                if (
                    len(set(self._equivalents.keys()) & self.automorphs[idx])
                    == 0
                ):
    ...
    def _build_automorphism(self):
        """Automorphisms are 0-based prune the automorphism map to remove
        identities and compact multiple mappings."""
